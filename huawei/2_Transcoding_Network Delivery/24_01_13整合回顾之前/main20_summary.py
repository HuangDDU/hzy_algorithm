...
from main7_map import Provider, Consumer
from main7_map import Node as OldNode
...
class Node(OldNode):
    def __init__(self, x, y, weight) -> None:
        ...
        self.best_direction_list = [] # 可能会有多个同等代价的候选前驱，后续需要选择代价最低的
        ...
        while not q.empty():
            ...
            if current_node.node_type == "CONSUMER":
                ...
                for (neighbor_x, neighbor_y), direction in map.get_neighbor_list(current_x, current_y):
                    ...
                    if not current_node.node_type == "PROVIDER":
                        turn = True
                        for d in current_node.best_direction_list:
                            if direction == d:
                                # 有一个候选方向没有转弯，最好的路径
                                turn = False
                                current_node.best_direction = d # TODO: 这样直接更改前驱是不是有问题
                                break
                        if turn:
                            new_cost += self.per_transmitter_cost
                    if (neighbor_node.distance==-1) or new_cost <= neighbor_node.distance:
                        ...
                        neighbor_node.best_direction_list.append(direction)
            current_node.visited = True 
...
if __name__ == "__main__":
    ...
    # main(conf_file="main20_summary.txt")
    main(conf_file="main20_summary2.txt")
