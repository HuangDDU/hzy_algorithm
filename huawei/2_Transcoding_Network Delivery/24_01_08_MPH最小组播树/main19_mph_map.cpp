...
public:
  ...
  int heuristic(int x,int y, vector<int> consumer_index_list){
    // 到达所有结点最近的曼哈顿距离,
    // TODO: 直接曼哈顿距离用到路径权值有点问题，需要手动乘以缩放系数
    vector<int> manhattan_distance_list;
    for(int consumer_index: consumer_index_list){
      int consumer_x = consumer_index/this->N, consumer_y= consumer_index%this->N;
      int manhattan_distance =  abs(x-consumer_x) + abs(y-consumer_y);
  ...
  pair<map<pair<int, int>, vector<pair<int, int>>>, map<pair<int, int>, int>> dijkstra(Map &node_map, int source, vector<int> consumer_index_list){
    ...
    vector<vector<vector<direction>>> direction_list_matrix(this->N, vector<vector<direction>>(this->N, vector<direction>())); // 可能会有多个同等代价的候选前驱，后续需要选择代价最低的
    ...
    while(!q.empty()){
      ...
      if(!(it==consumer_index_list.end())){
        ...
        for(auto p: node_map.get_neighbor_list(current_index)){
          ...
          // 添加转弯代价
          if (!(current_index==source)){
            bool turn = true;
            for(direction tmp_d: direction_list_matrix[current_x][current_y]){
              if(d==tmp_d){
                // 有一个候选方向没有转弯，最好的路径
                turn = false;
          ...
          if ((distance_matrix[neighbor_x][neighbor_y] == -1) or (new_cost <= distance_matrix[neighbor_x][neighbor_y])){
            // 更新从当前结点其到的源点距离
            int priority = new_cost + heuristic(neighbor_x, neighbor_y, consumer_index_list); // A*改进
            ...
            direction_list_matrix[neighbor_x][neighbor_y].push_back(d); //添加候选一个方向
          }
        }
      }
       visited_matrix[current_x][current_y] = true;
