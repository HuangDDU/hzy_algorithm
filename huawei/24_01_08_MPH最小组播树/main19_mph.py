...
class Controller():
    ...
    def dijkstra(self, map, source, consumer_index_list):
        # 初始化
        ...
        for consumer_index in consumer_index_list:
            ...
            cost_dict[(source, consumer_index)] = distance_matrix[int(consumer_index/self.N)][consumer_index%self.N]
        return path_dict, cost_dict


    def mph(self, map, provider, consumer_vector):
        # 坐标转化为序号
        consumer_index_list = [consumer.x*self.N+consumer.y for consumer in consumer_vector]
        ...
        for i in range(len(consumer_index_list)):
            ...
            for new_V in new_V_list:
                new_path_dict, new_path_cost_dict = self.dijkstra(map, new_V, consumer_index_list)
