...
class Berth:
    def __init__(self, id, x=0, y=0, transport_time=0, loading_speed=0):
        ...
        # self.new_good_stack = Queue() # 归属于该泊位的货物
...
class Node:
    def __init__(self, x, y, type):
        ...
        self.berth_distance_list= [-1 for _ in range(berth_num)] # 到该泊位路径的长度
...
# robot2berth = {} # 1个robot对应的1个berth
berth2boat = {}
boat_id_count = 0
berth_id_list = berth2boat.keys()
for berth_id, robot_id_list in berth2robot.items():
    berth_item = berth[berth_id]
    berth2boat[berth_id] = boat_id_count # 对应的boat计数器
...
logger.debug(f"berth2boat : {berth2boat}")
...
def Init():
    ...
    for i in berth_id_list:
        ...
        for j in range(10):
            ...
            if node.berth_visited_list[i]==False:
                robot_j.available_list[i] = False # 不可到达
                logger.debug(f"robot({j}){(robot_j.x, robot_j.y)} to berth({i}){(berth_i.x, berth_i.y)} is unavailable")
    # 划分各个地图结点归属的泊位
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            if node.type == '.':
                ...
                berth_id = -1
                # 候选第一个不为-1的泊位ID
                for k in range(berth_num):
                    if not distance_list[k] == -1:
                        berth_id = k
                        break
                for k in range(berth_id, berth_num):
                    if (not distance_list[k] == -1) and (distance_list[k] < distance_list[berth_id]):
                        berth_id = k
                node.berth_id = berth_id
                row += str(berth_id)
            else:
                row += node.type
        logger.debug(row)
    ...
    for i in range(10):
        ...
        if robot_i.berth_id == -1 or node.berth_visited_list[robot_i.berth_id]==False:
            # 没有目标，或者不可到达
...
def Output():
    ...
    for i in berth_id_list:
        berth_i = berth[i]
        logger.debug(f"berth({i}) range good stack {berth_i.new_good_stack}") # 当前泊位归属的货物队列
        if not berth_i.boat_id == -1:
            if id >= max_frame - berth[i].transport_time - 10:
                # 最后快结束的时候，留出10帧的时间离开
